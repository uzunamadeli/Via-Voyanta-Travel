...
const Navbar: React.FC<NavbarProps> = ({ isDarkMode, toggleTheme, onOpenReservation, language, setLanguage }) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-24">
            ...
            <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
              <a href="#" className="text-slate-600 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400 font-medium transition-colors">{t.nav.home}</a>
              <a href="#tours" className="text-slate-600 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400 font-medium transition-colors">{t.nav.tours}</a>
              <a href="#hotels" className="text-slate-600 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400 font-medium transition-colors">{t.nav.hotels}</a>
              <a href="#tours" className="text-slate-600 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400 font-medium transition-colors">{t.nav.honeymoon}</a>
              <a href="#contact" className="text-slate-600 dark:text-slate-300 hover:text-sky-600 dark:hover:text-sky-400 font-medium transition-colors">{t.nav.contact}</a>
              
              <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-700 pl-4">
                  <button 
                    onClick={toggleLanguage}
                    className="p-2 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-1 font-bold text-sm"
            ...
            <div className="hidden md:flex">
              <button onClick={onOpenReservation} className="bg-blue-950 dark:bg-sky-600 text-white px-6 py-3 rounded-full font-medium hover:bg-sky-600 dark:hover:bg-sky-500 transition-all duration-300 shadow-lg hover:shadow-sky-500/30">
                {t.nav.book}
              </button>
            </div>
            ...
            <div className="md:hidden flex items-center gap-2">
               <button 
                onClick={toggleLanguage}
                className="p-2 rounded-full text-slate-600 dark:text-slate-300 font-bold text-xs"
               >
                 {language.toUpperCase()}
               </button>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-500 md:hidden ${isMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white dark:bg-slate-900 shadow-2xl transition-transform duration-500 ease-out transform flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
            ...
            {/* Partners Section */}
            <div>
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-2">{t.nav.partners}</h4>
            ...
            <div id="mobile-contact">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-2">{t.nav.contact}</h4>
          ...
          {/* Footer CTA */}
          <div className="p-6">
            <button onClick={handleReservationClick} className="w-full bg-sky-500 text-white py-4 rounded-2xl font-bold shadow-xl shadow-sky-500/20 active:scale-95 transition-transform">
              {t.nav.book}
            </button>
          </div>
        </div>
